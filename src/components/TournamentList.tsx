...
export default function TournamentList({ onSelectTournament, onCreateTournament, onShowRegistration }: TournamentListProps) {
  ...
  const fetchRegistrationCounts = async (tournamentsList: Tournament[]) => {
    ...
    tournamentIds.forEach(id => {
      ...
      if (tournamentCats.length > 0) {
        const hasIndividualCategories = tournamentCats.some(c => c.format === 'individual_groups_knockout');
        if (hasIndividualCategories) {
          maxTeams[id] = Math.max(...tournamentCats.map(c => c.max_teams || 0));
        } else {
  ...
  const handleCopyTournament = async (e: React.MouseEvent, tournament: Tournament) => {
    ...
    const newName = prompt(`Enter name for the copied tournament:`, `${tournament.name} (Copy)`);
    if (!newName) return;

    const { data: categories } = await supabase
      .from('tournament_categories')
      .select('id, name, format, number_of_groups, max_teams')
    ...
    if (categories && categories.length > 0 && newTournament) {
      ...
      if (newCategories) {
        categories.forEach((oldCat, index) => {
          categoryIdMap.set(oldCat.id, newCategories[index].id);
        });
      }
    }
    ...
    if (newTournament) {
      ...
      if (players && players.length > 0) {
        ...
        if (newPlayers) {
          players.forEach((oldPlayer, index) => {
            playerIdMap.set(oldPlayer.id, newPlayers[index].id);
          });
        }
      }
      ...
      const { data: teams } = await supabase
        .from('teams')
        .select('id, name, player1_id, player2_id, seed, category_id')
        .eq('tournament_id', tournament.id);
      ...
      if (teams && teams.length > 0) {
        const teamsToInsert = teams.map(team => ({
          tournament_id: newTournament.id,
          name: team.name,
          player1_id: playerIdMap.get(team.player1_id) || team.player1_id,
          player2_id: playerIdMap.get(team.player2_id) || team.player2_id,
          seed: team.seed,
          category_id: team.category_id ? categoryIdMap.get(team.category_id) || null : null,
        }));
      ...
      const { data: individualPlayers } = await supabase
        .from('players')
        .select('id, name, email, phone_number, category_id')
        .eq('tournament_id', tournament.id)
        .is('team_id', null);

      if (individualPlayers && individualPlayers.length > 0) {
        const playersToInsert = individualPlayers.map(player => ({
          tournament_id: newTournament.id,
          name: player.name,
          email: player.email,
          phone_number: player.phone_number,
          category_id: player.category_id ? categoryIdMap.get(player.category_id) || null : null,
        }));
    ...
    fetchTournaments();
    alert('Tournament copied successfully!');
  };
